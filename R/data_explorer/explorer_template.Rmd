---
title: "Data explorer template"
knit: govukhugo::govukhugo_knit
date: 2021-01-01
section: data
weight: 999
summary: "Template for data explorer pages"
---
...
page_vars <- c(
  "status", "year",
  params$variables,
  "headcount", "fte", "mean_salary", "median_salary"
)
...
page_data <- explorer_data %>%
  filter(
    ...
    if_all(all_of(params$variables), ~.x != "Total")
  ) %>%
...
htmltools::tagList(
  htmltools::tags$div(
    ...
    htmltools::tags$ol(
      ...
      purrr::map2(
        .x = c("all", params$variables),
        .y = length(params$variables):0,
        ~breadcrumb(var = .x, depth = .y, var_labels = var_labels)
        )
    )
  )
)

if (length(params$variables) < 3) {
  htmltools::tags$div(
    class = "govuk-body-s",
