...
age_plot <- ggplot(age_chart_df,
  ...
  geom_col(fill = govukhugo::govuk_colours["dark-blue"]) +
  geom_col(aes(y = -bar_width), fill = govukhugo::govuk_colours["dark-blue"]) +
...
age_grade_plot <- ggplot(age_grade_plot_df,
  ...
  scale_fill_discrete(
    type = rev(scales::gradient_n_pal(govukhugo::govuk_palette("dark-blue"))(scales::rescale(1:5))),
    name = "Age band"
  ) +
...
age_org <- acses2021 %>%
  ...
  select(age_band, organisation, value) %>%
  mutate(
    chart_age_band = case_when(
      age_band == "16-19" | age_band == "20-29" ~ "16-29",
      age_band == "60-64" | age_band == "65 & over" ~ "60 & over",
